...
const PostCard = ({ post, layout = "horizontal", reverse = false }: PostProps) => {
  ...
  return (
    <Link className={`@container ${layout === "horizontal" ? "grid grid-cols-1 md:grid-cols-2 gap-10 items-center" : "space-y-10"}`} href={`/post/${post.slug}`}>
        <Image 
          className={`rounded-md w-full object-cover object-center h-full max-h-[300px] ${reverse ? "md:order-last" : ""}`} 
          // src={`${process.env.NEXT_PUBLIC_ASSETS_URL}${post.image}?key=optimages`}
